-- ============================================================
-- 074: Running Totals - cumulative sums over ordered data
-- ============================================================

CREATE TABLE IF NOT EXISTS sales (
    sale_id    INT PRIMARY KEY,
    sale_date  DATE,
    amount     DECIMAL(10,2)
);

INSERT INTO sales VALUES
(1, '2025-01-01', 1000),
(2, '2025-01-02', 1200),
(3, '2025-01-03',  800),
(4, '2025-01-04', 1500),
(5, '2025-01-05', 1100),
(6, '2025-01-06',  900),
(7, '2025-01-07', 1300);

-- Simple running total (cumulative sum)
SELECT sale_date, amount,
       SUM(amount) OVER (ORDER BY sale_date) AS running_total
FROM sales
ORDER BY sale_date;

-- Running total with row number
SELECT sale_date, amount,
       ROW_NUMBER() OVER (ORDER BY sale_date) AS day_num,
       SUM(amount) OVER (ORDER BY sale_date) AS running_total
FROM sales
ORDER BY sale_date;

-- Running total by month
WITH monthly AS (
    SELECT STRFTIME('%Y-%m', sale_date) AS month,
           SUM(amount) AS monthly_total
    FROM sales
    GROUP BY month
)
SELECT month, monthly_total,
       SUM(monthly_total) OVER (ORDER BY month) AS running_total
FROM monthly
ORDER BY month;

-- Running count
SELECT sale_date, amount,
       COUNT(*) OVER (ORDER BY sale_date) AS running_count
FROM sales
ORDER BY sale_date;

-- Running average (changing average over time)
SELECT sale_date, amount,
       AVG(amount) OVER (ORDER BY sale_date) AS running_avg
FROM sales
ORDER BY sale_date;

-- Running total with percentage of total
SELECT sale_date, amount,
       SUM(amount) OVER (ORDER BY sale_date) AS running_total,
       SUM(amount) OVER () AS grand_total,
       ROUND(SUM(amount) OVER (ORDER BY sale_date) * 100.0 / SUM(amount) OVER (), 2) AS pct_of_total
FROM sales
ORDER BY sale_date;

-- Running total with partition
CREATE TABLE IF NOT EXISTS product_sales (
    sale_id   INT PRIMARY KEY,
    product   VARCHAR(50),
    sale_date DATE,
    amount    DECIMAL(10,2)
);
INSERT INTO product_sales VALUES
(1, 'Laptop', '2025-01-01', 1200),
(2, 'Laptop', '2025-01-02', 1100),
(3, 'Laptop', '2025-01-03', 900),
(4, 'Mouse',  '2025-01-01', 25),
(5, 'Mouse',  '2025-01-02', 30),
(6, 'Mouse',  '2025-01-03', 20);

SELECT product, sale_date, amount,
       SUM(amount) OVER (PARTITION BY product ORDER BY sale_date) AS running_total
FROM product_sales
ORDER BY product, sale_date;

-- Cleanup
DROP TABLE IF EXISTS product_sales;
DROP TABLE IF EXISTS sales;
